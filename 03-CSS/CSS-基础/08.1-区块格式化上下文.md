...
- 匿名表格单元格元素（display 值为 table（HTML 表格默认值）、table-row（表格行默认值）、table-row-group（表格体默认值）、table-header-group（表- 格头部默认值）、table-footer-group（表格尾部默认值）或 inline-table）。
...
[包含内部浮动](Code/溢出/包含内部浮动.html)

[排除外部浮动](Code/溢出/排除外部浮动.html)

[防止外边距重叠](Code/溢出/防止外边距重叠.html)